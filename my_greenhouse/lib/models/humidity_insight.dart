...
class _HumidityInsightState extends State<HumidityInsight> {
  ...
  void initState() {
    ...
    if (firebaseService.humidityDataPoints[TimeUnit.second]!.isEmpty) {
      for (var unit in TimeUnit.values) {
        firebaseService.humidityDataPoints[unit] = [];
            ...
            child: Padding(
              ...
              child: LineChart(
                LineChartData(
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: firebaseService.humidityDataPoints[selectedTimeUnit]!,
                      isCurved: true,
                      ...
                      dotData: FlDotData(
                        ...
                        getDotPainter: (spot, percent, barData, index) {
                          if (index ==
                              firebaseService
                                  .humidityDataPoints[selectedTimeUnit]!
                                  .length -
                                  1) {
  ...
  void updateDataPoints(SensorData newData) {
    ...
    for (var unit in TimeUnit.values) {
      if (shouldUpdateUnit(unit, now)) {
        if (firebaseService.humidityDataPoints[unit]!.length >= maxDataPoints) {
          firebaseService.humidityDataPoints[unit]!.removeAt(0);
          for (int i = 0; i < firebaseService.humidityDataPoints[unit]!.length; i++) {
            firebaseService.humidityDataPoints[unit]![i] = FlSpot(i.toDouble(), firebaseService.humidityDataPoints[unit]![i].y);
          }
        }
        firebaseService.humidityDataPoints[unit]!.add(FlSpot(firebaseService.humidityDataPoints[unit]!.length.toDouble(), newData.humidity));
  ...
  void aggregateDataPoints() {
    for (var unit in TimeUnit.values) {
      ...
      final lowerUnitData = firebaseService.humidityDataPoints[lowerUnit]!;
      ...
      firebaseService.humidityDataPoints[unit] = aggregatedData;
    }
  }
