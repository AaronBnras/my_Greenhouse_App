...
class _TemperatureInsightState extends State<TemperatureInsight> {
  ...
  void initState() {
    ...
    if (firebaseService.temperatureDataPoints[TimeUnit.second]!.isEmpty) {
      for (var unit in TimeUnit.values) {
        firebaseService.temperatureDataPoints[unit] = [];
                        ...
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color: (sensorData.temperatureC <= 10 ||
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(
                                  color: (sensorData.temperatureF <= 50 ||
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: firebaseService
                          .temperatureDataPoints[selectedTimeUnit]!,
                      isCurved: true,
  ...
  void updateDataPoints(SensorData newData) {
    ...
    for (var unit in TimeUnit.values) {
      if (shouldUpdateUnit(unit, now)) {
        if (firebaseService.temperatureDataPoints[unit]!.length >=
            maxDataPoints) {
          firebaseService.temperatureDataPoints[unit]!.removeAt(0);
          // Shift all x values to the left
          for (int i = 0;
              i < firebaseService.temperatureDataPoints[unit]!.length;
              i++) {
            firebaseService.temperatureDataPoints[unit]![i] = FlSpot(
                i.toDouble(),
                firebaseService.temperatureDataPoints[unit]![i].y);
          }
        }
        firebaseService.temperatureDataPoints[unit]!.add(FlSpot(
            firebaseService.temperatureDataPoints[unit]!.length.toDouble(),
            newData.temperatureC));
  ...
  void aggregateDataPoints() {
    for (var unit in TimeUnit.values) {
      ...
      final lowerUnitData = firebaseService.temperatureDataPoints[lowerUnit]!;
      ...
      firebaseService.temperatureDataPoints[unit] = aggregatedData;
    }
  }
