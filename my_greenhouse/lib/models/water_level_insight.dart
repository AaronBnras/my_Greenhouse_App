...
class _WaterLevelInsightState extends State<WaterLevelInsight> {
  ...
  void initState() {
    ...
    if (firebaseService.waterLevelDataPoints[TimeUnit.second]!.isEmpty) {
      for (var unit in TimeUnit.values) {
        firebaseService.waterLevelDataPoints[unit] = [];
                        ...
                        child: Row(
                          children: [
                            Padding(
                              padding: const EdgeInsets.all(1.5),
                              child: Text(
                                'Water Level : ${sensorData.waterLevel.toStringAsFixed(1)}% ',
                                overflow: TextOverflow.ellipsis,
                                style: TextStyle(
                                  color: sensorData.waterLevel < 30
                                      ? Colors.red.shade400
                  ...
                  lineBarsData: [
                    LineChartBarData(
                      spots: firebaseService
                          .waterLevelDataPoints[selectedTimeUnit]!,
                      isCurved: true,
  ...
  void updateDataPoints(SensorData newData) {
    ...
    for (var unit in TimeUnit.values) {
      if (shouldUpdateUnit(unit, now)) {
        if (firebaseService.waterLevelDataPoints[unit]!.length >=
            maxDataPoints) {
          firebaseService.waterLevelDataPoints[unit]!.removeAt(0);
          for (int i = 0;
              i < firebaseService.waterLevelDataPoints[unit]!.length;
              i++) {
            firebaseService.waterLevelDataPoints[unit]![i] = FlSpot(
                i.toDouble(), firebaseService.waterLevelDataPoints[unit]![i].y);
          }
        }
        firebaseService.waterLevelDataPoints[unit]!.add(FlSpot(
            firebaseService.waterLevelDataPoints[unit]!.length.toDouble(),
            newData.waterLevel));
  ...
  void aggregateDataPoints() {
    for (var unit in TimeUnit.values) {
      ...
      final lowerUnitData = firebaseService.waterLevelDataPoints[lowerUnit]!;
      ...
      firebaseService.waterLevelDataPoints[unit] = aggregatedData;
    }
  }
